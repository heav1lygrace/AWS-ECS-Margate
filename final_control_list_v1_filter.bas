...
Sub BuildSecurityTool()
    ...
    ' Auto-filter to show only applicable
    ws.Range("A1:M" & lastCtrl).AutoFilter
    ws.Range("A1:M" & lastCtrl).AutoFilter Field:=13, Criteria1:="Yes"
    ws.Range("A1:M" & lastCtrl).AutoFilter Field:=12, Criteria1:="Yes"

    ' Hide the Enabled and Applicable columns
...
Sub RefreshChecklist()
    Dim ws As Worksheet
    Set ws = ActiveWorkbook.Sheets("Security Checklist")
    Application.Calculate
    If ws.AutoFilterMode Then ws.AutoFilter.ShowAllData
    Dim lr As Long
    lr = ws.Cells(ws.Rows.Count, 2).End(xlUp).Row
    If Not ws.AutoFilterMode Then
        ws.Range("A1:M" & lr).AutoFilter
    End If
    ...
    MsgBox "Checklist refreshed. Showing applicable controls only.", _
        vbInformation
End Sub
